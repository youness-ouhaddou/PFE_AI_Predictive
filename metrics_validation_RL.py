...
# =====================================================================
# ─── CALCUL DES MÉTRIQUES DÉTAILLÉES ET GRAPHIQUES (A AJOUTER A LA FIN)
# =====================================================================
import math
...
# Les variables X_test, y_test et model_lr sont déjà connues dans ce script
y_pred_vals = model_lr.predict(X_test)
...
# Calcul sécurisé du MAPE
mape = np.mean(np.abs(
    (y_test_vals - y_pred_vals) / 
    np.where(y_test_vals == 0, 1, y_test_vals)
)) * 100

print("\n╔══════════════════════════════════════════╗")
print("║  MÉTRIQUES — RÉGRESSION LINÉAIRE PDR     ║")
print("╚══════════════════════════════════════════╝")
print(f"  MAE  : {mae:.4f} pièces/mois (Erreur moyenne)")
print(f"  RMSE : {rmse:.4f} pièces/mois (Pénalise les grands écarts)")
print(f"  R²   : {r2:.4f} (Variance expliquée)")
print(f"  MAPE : {mape:.1f} % (Erreur relative moyenne)")

# ─── Graphiques métriques régression ───
fig, axes = plt.subplots(1, 2, figsize=(13, 5))
fig.suptitle('Métriques de validation — Régression Linéaire PDR', fontsize=13, fontweight='bold')

# Prédit vs Réel
ax1 = axes[0]
ax1.scatter(y_test_vals, y_pred_vals, alpha=0.6, color='#3498db', s=50, edgecolors='none', label='Prédictions')
min_v = min(np.min(y_test_vals), np.min(y_pred_vals)) - 0.5
max_v = max(np.max(y_test_vals), np.max(y_pred_vals)) + 0.5
ax1.plot([min_v, max_v], [min_v, max_v], 'r--', linewidth=1.5, label='Prédiction parfaite')
ax1.set_xlabel('Valeurs Réelles (pièces/mois)', fontsize=11)
ax1.set_ylabel('Valeurs Prédites (pièces/mois)', fontsize=11)
...
ax2.hist(residuals, bins=12, color='#9b59b6', edgecolor='white', alpha=0.8)
ax2.axvline(x=0, color='red', linestyle='--', linewidth=1.5, label='Résidu idéal = 0')
ax2.axvline(x=mae, color='orange', linestyle='--', linewidth=1.5, label=f'MAE = +{mae:.2f}')
ax2.axvline(x=-mae, color='orange', linestyle='--', linewidth=1.5, label=f'MAE = −{mae:.2f}')
ax2.set_xlabel('Résidu (Réel − Prédit)', fontsize=11)
...
plt.tight_layout()
plt.savefig('outputs/graphs/metriques_regression.png', dpi=150, bbox_inches='tight')
print("\n✅ metriques_regression.png sauvegardé")
plt.show()